...
def unet_resinc( inp_layer,ksize=3,depth=None,filters=32,layers=None,keep_prob=1.0,num_classes=2,lite=False ):
	...
	for i in range(0,depth+1):
		...
		### Add Residual layers
		for l in range(0,layers):
			conv = ResInc_v1( conv, filters=fm[-1], ksize=ksize )
			print(conv)
	...
	for i in range(depth):
		...
		### Add Residual layers
		for l in range(0,layers):
			conv = ResInc_v1( conv, filters=fm[i]*2, ksize=ksize )
			print(conv)
...
# DEF UNET_RESINC
def unet_resinc_old( inp_layer,depth=4,ksize=3,filters=32,layers=1,keep_prob=1.0,num_classes=2,lite=False ):
...
# DEF UNET
def unet_resinc_new_new( inp_layer,ksize=3,depth=None,filters=32,layers=None,keep_prob=1.0,num_classes=2,lite=False ):
	...
	### Project the input layer dimensions from 2 features to f features
	### This ensures the input layer size goes from [?,x,y,2] -> [?,x,y,f]
	conv = conv_layer_resample_v1( inp=inp_layer, filters=fm[-1], ksize=1, stride=1, \
					upsample=False )
	print(conv)
	...
	### Add Residual layers
	for l in range(0,layers):
		conv = ResInc_v1( conv, filters=filters, ksize=ksize )
		print(conv)
	...
	for i in range(0,depth+1):
		...
		if i>0:
			# IF LITE NETWORK
			if lite:
#				fm.append( min( 1024,fm[-1]+filters ) )
				fm.append( fm[-1]+filters )
			else:
#				fm.append( min( 1024,fm[-1]*2 ) )
				fm.append( fm[-1]*2 )
			# ELIF LITE NETWORK
	...
	for i in range(depth):
		...
		### Upsample once
		conv = conv_layer_resample_v1( inp=conv, filters=fm[i], ksize=2, stride=2, \
						upsample=True )
		print(conv)
	...
	### Add Residual layers
	for l in range(0,layers):
		conv = ResInc_v1( conv, filters=conv.shape.as_list()[-1], ksize=ksize )
		print(conv)
...
# DEF UNET
def unet_resinc_x3_new( inp_layer,ksize=3,depth=None,filters=32,layers=None,keep_prob=1.0,num_classes=2,lite=False ):
...
#				fm.append( min( 1024,fm[-1]*2 ) )
		...
		### Add Residual layers
		for l in range(0,layers):
			conv = ResInc_x3_v1( conv, filters=fm[-1], ksize=ksize )
			print(conv)
	...
	for i in range(depth):
		...
		### Add Residual layers
		for l in range(0,layers):
			conv = ResInc_x3_v1( conv, filters=fm[i]*2, ksize=ksize )
			print(conv)
